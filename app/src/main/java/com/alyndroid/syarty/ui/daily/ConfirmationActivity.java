...
import com.alyndroid.syarty.ui.newRequest.NewRequestView;
            ...
            try {
                ...
                switch (requestCode) {
                    case Constant.permission.right_side_IMAGE:
                        rightSideFile = CommonUtils.scaleDown(capturedImageFile, 300, false, bitmap);
                        Picasso.get().load(uri).into(rightSideIv);
                        break;
                    case Constant.permission.back_side_IMAGE:
                        backSideFile = CommonUtils.scaleDown(capturedImageFile, 300, false, bitmap);
                        Picasso.get().load(uri).into(backsideIv);
                        break;
                    case Constant.permission.left_side_IMAGE:
                        leftSideFile = CommonUtils.scaleDown(capturedImageFile, 300, false, bitmap);
                        Picasso.get().load(uri).into(leftSideIv);
                        break;
                    case Constant.permission.front_side_IMAGE:
                        frontSideFile = CommonUtils.scaleDown(capturedImageFile, 300, false, bitmap);
